...
describe('Auth', () => {
  ...
  describe('POST /auth/refresh', () => {
    ...
    test('When refresh token is valid, expect it returns new access token', async () => {
      const role: UserRole = UserRole.AUTHOR;
      const email: string = `${v4()}@email.com`;
      const password: string = v4();
      
      const user: User = await userFixture.insertUser({role, email, password});
      const auth: {accessToken: string, refreshToken: string} = await AuthFixture.loginUser({id: user.getId()});
      ...
      ResponseExpect.codeAndMessage(response.body, {code: Code.SUCCESS.code, message: Code.SUCCESS.message});
      ResponseExpect.data({response: response.body}, {accessToken: expect.any(String)});
      
      expect(response.body.data.accessToken).toBeDefined();
      expect(response.body.data.accessToken).not.toBe(auth.accessToken); // Should be different token
    });
  
    test('When refresh token is invalid, expect it returns "UNAUTHORIZED_ERROR" response', async () => {
      const response: supertest.Response = await supertest(testServer.serverApplication.getHttpServer())
        .post('/auth/refresh')
    ...
    test('When refresh token is missing, expect it returns "UNAUTHORIZED_ERROR" response', async () => {
      const response: supertest.Response = await supertest(testServer.serverApplication.getHttpServer())
        .post('/auth/refresh')
        .send({})
        .expect(HttpStatus.OK);
